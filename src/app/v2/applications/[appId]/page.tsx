  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link href={`/v2/applications/${appId}/edit`} className="btn-secondary text-sm">Edit</Link>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href={`/v2/applications/${appId}/projects/new`} className="btn-primary text-sm">+ New Project</Link>
        </div>
        <div className="space-y-2">
          {projects.map(proj => {
            const specs = listSpecConfigs(appId, proj.id)
            return (
              <Link key={proj.id} href={`/v2/applications/${appId}/projects/${proj.id}`}
          ...
          {projects.length === 0 && (
            <div className="card text-center py-8 text-gray-500 text-sm">
              No projects yet. <Link href={`/v2/applications/${appId}/projects/new`} className="text-blue-600 hover:underline">Create one</Link>
            </div>
          )}
        </div>
      </div>
