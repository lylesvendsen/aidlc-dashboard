...
export default function EditApplicationPage() {
  ...
  useEffect(() => {
    fetch(`/api/v2/applications/${appId}`).
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/v2/applications/${appId}`, {
        method: "PUT",
      ...
      router.push(`/v2/applications/${appId}`)
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
      <nav className="text-sm text-gray-500">
        ...
        <Link href={`/v2/applications/${appId}`} className="hover:text-gray-900">{form.name}</Link>
        <span className="mx-2">/</span>
      ...
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href={`/v2/applications/${appId}`} className="btn-ghost">Cancel</Link>
        </div>
      </form>
    </div>
  )
