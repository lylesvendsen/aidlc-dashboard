...
export default function SpecEditorPage() {
  ...
  useEffect(() => {
    fetch("/api/v2/specs/" + specId + "?appId=" + appId + "&projId=" + projId)
      .then(r => r.json())
  ...
  const save = async () => {
    setSaving(true)
    await fetch("/api/v2/specs/" + specId + "?appId=" + appId + "&projId=" + projId, {
      method: "PUT",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={`/v2/applications/${appId}/projects/${projId}/run?specFile=` + encodeURIComponent(spec.filePath)} className="btn-primary">
            Run Spec
          </Link>
          <button onClick={save} disabled={saving} className="btn-secondary">
            {saving ? "Saving..." : saved ? "Saved!" : "Save"}
          </button>
          <button onClick={() => router.push(`/v2/applications/${appId}/projects/${projId}`)} className="btn-ghost">Back</button>
        </div>
      </div>
