...
function ProjectForm({ isNew = false }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/v2/applications/${appId}`).then(r => r.json()).then(d => setAppName(d.name ?? ""))
    if (!isNew && projId) {
      fetch(`/api/v2/applications/${appId}/projects/${projId}`).then(r => r.json()).then(d => {
        setName(d.name ?? "")
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      let res: Response
      if (isNew) {
        res = await fetch(`/api/v2/applications/${appId}/projects`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        })
      } else {
        res = await fetch(`/api/v2/applications/${appId}/projects/${projId}`, {
      ...
      router.push(`/v2/applications/${appId}/projects/${data.id}`)
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
      <nav className="text-sm text-gray-500">
        ...
        <Link href={`/v2/applications/${appId}`} className="hover:text-gray-900">{appName}</Link>
        <span className="mx-2">/</span>
        {!isNew && projId && <><Link href={`/v2/applications/${appId}/projects/${projId}`} className="hover:text-gray-900">{name}</Link><span className="mx-2">/</span></>}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <Link href={isNew ? `/v2/applications/${appId}` : `/v2/applications/${appId}/projects/${projId}`} className="btn-ghost">Cancel</Link>
        </div>
      </form>
    </div>
  )
